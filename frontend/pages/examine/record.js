...
const record = () => {
  ...
  useEffect(() => {
    ...
    const fetchdata = async () =>{
      try{
        const gn = await Axios.post(process.env.NEXT_PUBLIC_API_BASE_URL + `/api/getGroupName`,load);
        console.log(gn.data)
        setGname(gn.data)
        setShow(gn.data)
        console.log(gn.data);
        ...
        //   }
          // const d = await Axios({
          //   // url : process.env.NEXT_PUBLIC_API_BASE_URL + `/api/getGroupNameSt`,
          //   // method : 'post',
          //   // data :  load  ,
          // })
  ...
  const clickDeleted = (idx , groupName) => {
    ...
    Axios.post(process.env.NEXT_PUBLIC_API_BASE_URL + "/api/DeleteGroupName" ,load)
  }
  return (
    <div>
